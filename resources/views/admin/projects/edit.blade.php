...
@section('page-title', __('Edit') . ': ' . $project->localized_title)
...
@section('content')
<form action="{{ route('admin.projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" 
                           ...
                           value="{{ old('expected_beneficiaries_count', $project->expected_beneficiaries_count) }}"
                           min="1"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="location_country"
                           value="{{ old('location_country', $project->location_country) }}"
                ...
                <div>
                    ...
                    <input type="text" 
                           ...
                           id="location_region"
                           value="{{ old('location_region', $project->location_region) }}"
                ...
                <div>
                    ...
                    <input type="text" 
                           name="location_city" 
                           id="location_city"
                           value="{{ old('location_city', $project->location_city) }}"
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="expected_impact_ar" 
                              ...
                              placeholder="{{ __('Describe the expected impact in Arabic...') }}">{{ old('expected_impact_ar', $project->expected_impact_ar) }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="expected_impact_en" 
                              ...
                              placeholder="{{ __('Describe the expected impact in English...') }}">{{ old('expected_impact_en', $project->expected_impact_en) }}</textarea>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="status" 
                        ...
                        <option value="paused" {{ old('status', $project->status) === 'paused' ? 'selected' : '' }}>{{ __('Paused') }}</option>
                        <option value="completed" {{ old('status', $project->status) === 'completed' ? 'selected' : '' }}>{{ __('Completed') }}</option>
                        <option value="closed" {{ old('status', $project->status) === 'closed' ? 'selected' : '' }}>{{ __('Closed') }}</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-6 flex items-center">
                <i class="fas fa-photo-video mx-2 text-purple-500"></i>
                {{ __('Current Images') }}
            </h3>
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-3">
                    ...
                    <button type="button" 
                            ...
                            class="inline-flex items-center px-6 py-3 border border-green-300 rounded-lg shadow-sm bg-green-50 text-sm font-medium text-green-700 hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                    ...
                    <button type="button" 
                            ...
                            class="inline-flex items-center px-6 py-3 border border-red-300 rounded-lg shadow-sm bg-red-50 text-sm font-medium text-red-700 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
...
document.getElementById('title_en').addEventListener('input', function() {
    ...
    if (!document.getElementById('slug').value || document.getElementById('slug').value === '{{ $project->slug }}') {
...
function deleteImage(mediaId) {
    if (confirm('{{ __("Are you sure you want to delete this image?") }}')) {
        fetch(`/admin/projects/{{ $project->id }}/media/${mediaId}`, {
            method: 'DELETE',
            headers: {
...
function deleteProject() {
    if (confirm('{{ __("Are you sure you want to delete this project? This action can be undone.") }}')) {
...
function restoreProject() {
    if (confirm('{{ __("Are you sure you want to restore this project?") }}')) {
        fetch('{{ route("admin.projects.restore", $project->id) }}', {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Content-Type': 'application/json'
        ...
        .then(data => {
            if (data.success) {
                ...
                alert('{{ __("Error restoring project") }}');
            }
        })
        .catch(error => {
            console.error('Error:', error);
